...
def build_nasbench360_deepsea_dataset(cfg):
    ...
    train_data, train_labels = (
        torch.from_numpy(data["x_train"]).type(torch.FloatTensor),
    ...
    val_data, val_labels = (
        torch.from_numpy(data["x_val"]).type(torch.FloatTensor),
    ...
    test_data, test_labels = (
        torch.from_numpy(data["x_test"]).type(torch.FloatTensor),
