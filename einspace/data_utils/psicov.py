...
def get_bulk_output_contact_maps(pdb_id_list, all_dist_paths, OUTL):
    ...
    if np.any(np.isnan(Y)):
        ...
        print("WARNING:")
        print(
            "Some pdbs in the following list have NaNs in their distances:", pdb_id_list
        )
...
def get_input_output_dist(
    pdb_id_list, all_feat_paths, all_dist_paths, pad_size, OUTL, expected_n_channels
):
    ...
    for i, pdb in enumerate(pdb_id_list):
        ...
        if len(X[:, 0, 0]) != len(Y0[:, 0]):
            print("")
            print(
                "WARNING!! Different len(X) and len(Y) for ",
                pdb,
                len(X[:, 0, 0]),
                len(Y0[:, 0]),
            )
        len_input = len(X[:, 0, 0])
        Y = np.full((len_input, len_input), np.nan)
        Y[: len(Y0[:, 0]), : len(Y0[:, 0])] = Y0
        Xpadded = np.zeros(
            (len_input + pad_size, len_input + pad_size, len(X[0, 0, :])),
            dtype=np.float32,
        )
        Xpadded[
            int(pad_size / 2) : len_input + int(pad_size / 2),
            int(pad_size / 2) : len_input + int(pad_size / 2),
            :,
        ] = X
        Ypadded = np.full(
            (len_input + pad_size, len_input + pad_size), 100.0, dtype=np.float32
        )
        Ypadded[
            int(pad_size / 2) : len_input + int(pad_size / 2),
            int(pad_size / 2) : len_input + int(pad_size / 2),
        ] = Y
        len_padded_input = len(Xpadded[:, 0, 0])
        if len_padded_input <= OUTL:
            XX[i, :len_padded_input, :len_padded_input, :] = Xpadded
            YY[i, :len_padded_input, :len_padded_input, 0] = Ypadded
        else:
            rx = random.randint(0, len_padded_input - OUTL)
            ry = random.randint(0, len_padded_input - OUTL)
            assert rx + OUTL <= len_padded_input
            assert ry + OUTL <= len_padded_input
            XX[i, :, :, :] = Xpadded[rx : rx + OUTL, ry : ry + OUTL, :]
            YY[i, :, :, 0] = Ypadded[rx : rx + OUTL, ry : ry + OUTL]
    return XX.astype(np.float32), YY.astype(np.float32)


def get_input_output_bins(
    pdb_id_list,
    all_feat_paths,
    all_dist_paths,
    pad_size,
    ...
    for i, pdb in enumerate(pdb_id_list):
        ...
        len_input = len(X[:, 0, 0])
        Y = np.full((len_input, len_input, len(Y0[0, 0, :])), np.nan)
        Y[: len(Y0[:, 0]), : len(Y0[:, 0]), :] = Y0
        Xpadded = np.zeros(
            (len_input + pad_size, len_input + pad_size, len(X[0, 0, :]))
        )
        ...
        Ypadded = np.full((len_input + pad_size, len_input + pad_size, len(bins)), 0.0)
        Ypadded[
            int(pad_size / 2) : len_input + int(pad_size / 2),
            int(pad_size / 2) : len_input + int(pad_size / 2),
            :,
        ...
        if len_padded_input <= OUTL:
            XX[i, :len_padded_input, :len_padded_input, :] = Xpadded
            YY[i, :len_padded_input, :len_padded_input, :] = Ypadded
        else:
            rx = random.randint(0, len_padded_input - OUTL)
            ry = random.randint(0, len_padded_input - OUTL)
            assert rx + OUTL <= len_padded_input
...
def get_feature(pdb, all_feat_paths, expected_n_channels):
    ...
    if features is None:
        ...
        exit(1)
    len_seq = len(features["seq"])
    # seq = features["seq"]
    # Create X and Y placeholders
    X = np.full((len_seq, len_seq, expected_n_channels), 0.0)
    # Add secondary structure
    ss = features["ss"]
    assert ss.shape == (3, len_seq)
    fi = 0
    for j in range(3):
        a = np.repeat(ss[j].reshape(1, len_seq), len_seq, axis=0)
        X[:, :, fi] = a
        fi += 1
        X[:, :, fi] = a.T
        fi += 1
    # Add PSSM
    pssm = features["pssm"]
    assert pssm.shape == (len_seq, 22)
    for j in range(22):
        a = np.repeat(pssm[:, j].reshape(1, len_seq), len_seq, axis=0)
        X[:, :, fi] = a
        fi += 1
        X[:, :, fi] = a.T
        fi += 1
    # Add SA
    sa = features["sa"]
    assert sa.shape == (len_seq,)
    a = np.repeat(sa.reshape(1, len_seq), len_seq, axis=0)
    X[:, :, fi] = a
    fi += 1
    X[:, :, fi] = a.T
    fi += 1
    ...
    assert entropy.shape == (len_seq,)
    a = np.repeat(entropy.reshape(1, len_seq), len_seq, axis=0)
    X[:, :, fi] = a
    fi += 1
    X[:, :, fi] = a.T
    fi += 1
    ...
    assert ccmpred.shape == ((len_seq, len_seq))
    X[:, :, fi] = ccmpred
    fi += 1
    # Add  FreeContact
    freecon = features["freecon"]
    assert freecon.shape == ((len_seq, len_seq))
    X[:, :, fi] = freecon
    fi += 1
    # Add potential
    potential = features["potential"]
    assert potential.shape == ((len_seq, len_seq))
    X[:, :, fi] = potential
    fi += 1
...
def get_map(pdb, all_dist_paths, expected_l=-1):
    ...
    if np.any(np.isnan(Y)):
        ...
        print("")
        print(
            "WARNING!! Some values in the pdb structure of",
            pdb,
            "l = ",
            ly,
            "are missing or nan! Indices are: ",
...
class DistGenerator:
    ...
    def __getitem__(self, index):
        batch_list = self.pdb_id_list[
            index * self.batch_size : (index + 1) * self.batch_size
        ]
        X, Y = get_input_output_dist(
            batch_list,
            self.features_path,
            self.distmap_path,
            self.pad_size,
...
def build_nasbench360_psicov_dataset(cfg, batch_size=4):
    ...
    all_feat_paths = [
        f"{path}/deepcov/features/",
        f"{path}/psicov/features/",
        f"{path}/cameo/features/",
    ]
    all_dist_paths = [
        f"{path}/deepcov/distance/",
        f"{path}/psicov/distance/",
        f"{path}/cameo/distance/",
    ]
    ...
    for pdb in deepcov_list:
        (ly, seqy, cb_map) = np.load(
            f"{path}/deepcov/distance/" + pdb + "-cb.npy", allow_pickle=True
        )
        length_dict[pdb] = ly
    ...
    for pdb in psicov_list:
        (ly, seqy, cb_map) = np.load(
            f"{path}/psicov/distance/" + pdb + "-cb.npy", allow_pickle=True
        )
    ...
    testset = PDNetDataset(
        my_list, all_feat_paths, all_dist_paths, 512, 10, 1, 57, label_engineering=None
    )
...
def calculate_mae(PRED, YTRUE):
    ...
    return (
        np.nanmean(mae_lr_d8_list),
        np.nanmean(mae_mlr_d8_list),
        np.nanmean(mae_lr_d12_list),
    ...
    with torch.no_grad():
        ...
        for batch in dataloader:
            ...
            for i in range(data.size(0)):
                targets.append(
                    np.expand_dims(target.cpu().numpy()[i].transpose(1, 2, 0), axis=0)
                )
