...
from einspace.data_utils.psicov import evaluate_test_protein
...
class Trainer:
    ...
    def __init__(
        ...
        self.score_fn = {
            ...
            "relative_l2": lambda x, y: -LpLoss(size_average=False)(x, y),
            "mae8": lambda x, y: -nn.MSELoss(reduction="mean")(torch.tensor(x), torch.tensor(y)).item(),
            "map": lambda x, y: calculate_map(
                torch.tensor(x),
                y,  # x is labels, y is predictions
            ),
        ...
        self.best = {
            "model": None,  # Don't store full model deepcopy - save memory
            "model_state": None,
            "train_score": 0,
    ...
    def train(self):
        ...
        lr_sampler = (
            (lambda: self.sample_log_uniform(-3, 0)) if self.config["lr"] is None else (lambda: self.config["lr"])
        )
        mom_sampler = (lambda: choice([0.9])) if self.config["momentum"] is None else (lambda: self.config["momentum"])
        wd_sampler = (
        ...
        for i in range(self.hpo_runs):
            ...
            print(
                "Training model with lr: {:.2e}, mom: {}, wd: {:.2e}".format(lr, mom, wd),
                flush=True,
            )
                ...
                try:
                    for data, target in self.train_dataloader:
                        ...
                        if self.score == "xe":
                            ...
                            predictions += torch.argmax(output.detach().cpu(), 1).tolist()
            ...
            # save the best overall model - store reference instead of deep copy
            # The actual model will be reconstructed when needed
            if valid_score > self.best["val_score"] or self.config["hpo_runs"] == 1:
                self.best["val_score"] = valid_score
                self.best["train_score"] = 0
                # Don't deepcopy the model - it will be reconstructed from architecture if needed
                # Store only the state dict which is more memory-efficient
                self.best["model_state"] = {k: v.cpu() for k, v in model.state_dict().items()}
                self.best["model"] = None  # Clear full model reference
                self.best["lr"] = lr
                self.best["momentum"] = mom
                self.best["weight_decay"] = wd
            ...
            # Memory cleanup: delete model and free GPU memory
    ...
    def evaluate(self, model, split="val"):
        ...
        if self.config["dataset"] == "psicov":
            ...
            def psicov_score_fn(x, y):
                """Compute negative criterion loss for psicov dataset."""
                return -self.criterion(torch.tensor(x), torch.tensor(y)).item()

            if split == "train":
                score_fn = psicov_score_fn
            elif split == "val":
                score_fn = psicov_score_fn
            elif split == "test":
